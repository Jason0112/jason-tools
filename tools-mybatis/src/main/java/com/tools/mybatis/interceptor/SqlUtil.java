...
//处理入参
    ...
    public void processOrderPageMappedStatement(MappedStatement ms, SqlSource sqlSource, Page page, Object[] args) {
        args[0] = getMappedStatement(ms, sqlSource, args[1], SUFFIX_ORDER, page);
    ...
    public static interface Parser {
        ...
        String getOrderPagerSql(String sql, Page page);
    ...
    public static abstract class SimpleParser implements Parser {
        ...
        @Override
        public String getOrderPagerSql(String sql, Page page) {
            StringBuilder sqlBuilder = new StringBuilder(sql.length() + 16);
            sqlBuilder.append(sql);
            sqlBuilder.append(" order by ");
            sqlBuilder.append(page.getSortField());
            sqlBuilder.append(" ");
...
//下面这段方法，主要解决一个常见类型的参数时的问题
    ...
    private class MyDynamicSqlSource implements SqlSource {
        ...
        public MyDynamicSqlSource(Configuration configuration, SqlNode rootSqlNode, Boolean count, Boolean orderBy, Page page) {
        ...
        public BoundSql getBoundSql(Object parameterObject) {
            ...
            if (count) {
                ...
                if (orderBy) {
                    sqlSource = getOrderPageSqlSource(configuration, sqlSource, parameterObject, page);
                } else {
...
//设置条件参数
     ...
     * @return
     */
    private MappedStatement getMappedStatement(MappedStatement ms, SqlSource sqlSource, Object parameterObject, String suffix, Page page) {
...
//创建一个新的MappedStatement
            qs = newMappedStatement(ms, getsqlSource(ms, sqlSource, parameterObject, suffix, page), suffix);
            try {
...
//count查询返回值int
     ...
     * @return
     */
    private SqlSource getsqlSource(MappedStatement ms, SqlSource sqlSource, Object parameterObject, String suffix, Page page) {
...
//如果是动态sql
        if (isDynamic(ms)) {
            ...
            return new MyDynamicSqlSource(ms.getConfiguration(), mixedSqlNode, suffix == SUFFIX_COUNT, suffix == SUFFIX_ORDER, page);
...
//改为分页sql
        ...
        } else if (suffix == SUFFIX_ORDER) {
            return getOrderPageSqlSource(ms.getConfiguration(), sqlSource, parameterObject, page);
        }
...
//如果是静态count-sql
     ...
     * @return
     */
    private SqlSource getOrderPageSqlSource(Configuration configuration, SqlSource sqlSource, Object parameterObject, Page page) {
        BoundSql boundSql = sqlSource.getBoundSql(parameterObject);
        return new StaticSqlSource(configuration, sqlParser.getOrderPagerSql(boundSql.getSql(), page), getPageParameterMapping(configuration, boundSql));
    }
